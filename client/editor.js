...
import {posFromIndex, getCursorIndex, getCursor} from './position.js';
import {markText, addLineClass, removeLineClass} from './decorations.js';
...
export default class Editor {
    ...
    getValue()   { return getValue(this.#view); }
    ...
    getCursorIndex() { return getCursorIndex(this.#view); }

    getCursor() { return getCursor(this.#view); }

    posFromIndex(index) { return posFromIndex(this.#view, index); }
    ...
    markRange(from, to, className) {
        return markText(this.#view, from, to, {className});
    }
    ...
    setMode(mode)   { return this.setOption('mode', mode); }
    setTheme(theme) { return this.setOption('theme', theme); }
    setKeyMap(name) { return this.setOption('keyMap', name); }

    setModeForPath(path) {
        return this.setMode(path.split('.').pop());
    }
    ...
    clearHistory() {
        clearHistory(this.#view, this.#view._historyCompartment, this.#vimEnabled);
        return this;
    }
